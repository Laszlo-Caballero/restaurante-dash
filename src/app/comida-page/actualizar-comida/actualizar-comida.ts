...
@Component({
  selector: 'app-actualizar-comida',
  imports: [
    Title,
    Button,
    InputComponent,
  ...
  templateUrl: './actualizar-comida.html',
})
export class ActualizarComida implements OnInit {
  private router = inject(Router);
  private httpClient = inject(HttpService);
  private authService = inject(AuthService);
  private imageUrl = ENV.imagesUrl;
  private activeRouter = inject(ActivatedRoute);
  private comidaId = signal<number | null>(null);
  cx = cx;
      ...
      this.httpClient
        ...
        .subscribe({
          next: (response) => {
            ...
            this.updateComida(this.comidaForm.value, recursoId);
          },
          error: () => {
  ...
  updateComida(comidaData: typeof this.comidaForm.value, recursoId: number) {
    this.httpClient
      .put(
        `comidas/${this.comidaId()}`,
        {
          ...comidaData,
          recursoId: recursoId,
        },
  ...
  loadComida() {
    this.isLoading = true;
    this.httpClient
      .get<ResponseApi<ComidaResponse>>(`comidas/${this.comidaId()}`, {
        headers: {
          Authorization: `Bearer ${this.authService.token}`,
        },
      })
      .subscribe({
        next: (response) => {
          ...
          this.comidaForm.setValue({
            ...
            categoriaIds: comida.categorias.map((cat) => cat.id),
            descripcion: comida.descripcion,
            disponible: comida.disponible,
          });
          this.selectedImage = {
            recursoId: comida.recurso.recursoId,
            nombre: comida.recurso.nombre,
          };
        },
      });
  }
